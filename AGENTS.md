...
- 설계 결정은 용어만 요약하지 않는다. **그 결정을 하지 않으면 생기는 구체적
  실패 장면**과 실제 값이 들어간 예시를 함께 설명한다. 여러 사용자 결정을
  받아야 하면 영향이 큰 것부터 한 번에 하나씩 묻는다.
- 실 AI 실행 비용은 `루프당 호출 수 × 예상 순환 수(범위)`로 추정한다. 완료 후
  실제 호출 수를 추정과 대조하고 차이의 원인을 기록한다 (ADR-0035).
- 소스만으로 upstream LLM 동작을 확정할 수 없으면 추측하지 않는다. 사용자는
  Ouroboros v0.50.8 실물 전사·화면을 제공할 수 있다. 이 자료는 `Observed`
  evidence이며 규범은 아니므로 pinned source와 함께 대조해 `docs/research/`에
  남긴다.
- 플러그인·MCP 등록처럼 사용자 환경을 바꾸는 검증용 설치는 측정한 같은 작업에서
  제거하고, 제거 여부까지 확인한다. 남겨야 한다면 먼저 사용자에게 묻는다.
- 도그푸딩 과제는 **관측하려는 기능은 포함하되 그 기능과 무관한 경계 사례 축은
  얕게** 고른다. 인코딩·파일명·CLI 파싱처럼 축이 깊은 과제는 필요할 때만 쓴다.
  단순화를 이유로 brownfield 검증을 greenfield 과제로 바꾸는 등 Phase의 관측
  대상을 제거해서는 안 된다 (DOGFOODING_0004·0005).
- 조사와 테스트가 통과해도 실제 입력의 모양이 다르면 결함이 남는다. 실 Runtime,
  CLI, 플러그인, brownfield 동작을 포함하는 Phase는 대표 실사용 경로의
  도그푸딩 없이 `COMPLETE`로 선언하지 않는다 (progress 0009 §1.2~§1.3).
