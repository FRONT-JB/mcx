...
- Phase 1~11 설계 결정은 ADR-0009~0053과 각 Phase
  기록에 연결되어 있다.
- Phase 10 source integration이 generation·ontology·content-key patch·세대별
  ...
  재계산해 source snapshot을 만든다. Mission record는 ADR-0037에 따라 Stage
  service가 읽지 않으며 `ACTIVE` 확인·전이 기록은 surface integration 소관이다.
- Wonder/Reflect text adapter는 도구 없는 `CompletionEngine` lane에서 parent
...
- 대표 brownfield Gen 2 도그푸딩이 실제 Verify `HOLD`→Wonder/Reflect→후속
  Blueprint→QA 1.00→exact user approval→Codex Execute→mechanical 3/3 +
...
- pinned upstream 병렬 경로를 재대조한 뒤 immutable dependency plan·grouped
  attempt durability·worker write Telemetry·mutating Coordinator·settled workspace
...
- Codex text+execution 단독 lifecycle 도그푸딩이 진행 중이다. Brief 실경로에서
  ...
  Brief는 `default` 무검증 반환과 `status_code: int` 밖 runtime type 제외까지
  확정됐으며 공개 시그니처에 타입 주석을 포함할지 결정 대기다
...
- 설치된 plugin에서 bare `mcx brief`가 선택되지 않고 `@Mission Control`만
  ...
  진입점에 대조했다. 여섯 skill description에 exact `mcx <stage>`를 앞세우고
  현재 Codex가 거부하는 `aliases` frontmatter를 제거했으며 plugin/marketplace를
  `0.1.1`로 맞췄다. 격리된 새 Codex 실행은 bare 입력에서 정확히
  `mcx:mcx-brief`를 선택했고, skill 6종 validation·Claude plugin validation·
  전체 **1068 tests**가 통과했다. `@Mission Control`은 결정적 명시 호출,
...
- Codex execution/text adapter의 실제 process spawn `OSError`는
  `RuntimeUnavailableError`로 표면화된다. Execute는 이를 `EXECUTION_FAILED`로
  저장하지 않고 순차·parallel attempt를 `DISPATCHED`에 남기며, CLI는 exit 1로
...
- `mcx --version`은 required stage subparser보다 먼저 실행되는 최상위
  argparse metadata option으로, `mission_control._version.__version__`을
  `mcx <version>` 형식으로 출력하고 exit 0으로 종료한다. ADR-0038 개정 3에
...
| `adr/` | 57 Accepted ADRs | ADR-0001~0057. Phase 11 병렬 Coordinator 실행, 저장소 CI 검증 게이트, durable state schema guard, runtime spawn 실패 경계까지 확정 |
| `research/` | Phase 11 complete + dogfood 0008 진행 중 | upstream baseline·도그푸딩 0001~0008·Evolve 연결·backend A/B·병렬 shared-worktree 안전 조사와 spawn/transport 오류 표면 대조 보존 |
...
### Phase 10 — Reflect/Evolve: 자가개선 루프 — COMPLETE (2026-08-10)
...
- [-] 구현
  - [x] domain/state vertical slice — `generation`·`ontology`·content-key AC
    patch·세대별 QA 예산·`EvolutionRecord` partial checkpoint·원자적
    successor append. fake Wonder/Reflect replay와 기존 JSON 호환을 테스트로
    고정 (`test_evolution.py`, `test_evolve_service.py`)
  - [x] 실 Blueprint·Execute·Verify state에서 `EvolveSourceSnapshot`을 구성하고
    Execute Gate `CLEAR`·Verify Gate `HOLD`, exact attempt·AC evidence lineage를
    LLM 호출 전에 재계산하는 application integration. 파일 저장소 round-trip
    integration 포함 (`test_evolve_flow.py`)
  - [x] 기존 vendor-neutral `CompletionEngine` Wonder/Reflect adapter — 도구 없는
    호출, grounded Wonder ref와 ordered Reflect index의 content-key 변환,
  ...
  - [x] Gen 2 대표 실사용 dogfood + Phase 10 종료 검토 — 실제 brownfield
    Verify `HOLD`에서 generation 2를 열고 revision 4 QA 1.00·exact user approval·
    Execute/Verify를 거쳐 `MISSION COMPLETE` ([DOGFOODING_0006](../research/DOGFOODING_0006.md),
    [progress 0010](./0010_REFLECT_EVOLVE.md))
...
- [x] 병렬 실행 도입 Gate 정의·적용 — 최초 `HOLD` 뒤 안전 조건 충족.
...
- [x] shared-worktree conflict authority — upstream Coordinator 경로 채택·구현
- [x] Gate `CLEAR` 뒤 staged batch 구현·대표 brownfield dogfood —
  `DOGFOODING_0007` MISSION COMPLETE
- [x] `REDISPATCH_ALT_HARNESS` 재평가 — 여전히 concrete execution Runtime이
  Codex 하나라 발동 조건이 없다. OpenCode adapter 실물 활성화 시점으로 이동
...
P2 이슈 #6의 `mcx --version` 표면 계약은 ADR-0038 개정 3에 기록하고
구현·회귀 테스트·로컬 전체 게이트·[GitHub Actions run
...
| Phase 11 — 병렬 실행 | **충족 (2026-08-10)** — upstream 22 focused tests 대조 뒤 worker write Telemetry + Coordinator 경로 채택. immutable plan·grouped attempts·bounded real fan-out·crash/quota HOLD·settled revalidation 구현. DOGFOODING_0007에서 worker 3개·공통 파일 conflict·Coordinator·independent Verify 3/3·MISSION COMPLETE. 이월 dirty rollback/candidate trace fixture 2건도 결함 수정 후 종료 | [progress 0011](./0011_PARALLEL_EXECUTION_GATE.md) |
