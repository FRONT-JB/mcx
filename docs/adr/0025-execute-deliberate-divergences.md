...
| 병렬 실행 (stage 안 병렬, serial-only 분리) | 존재 | v1 미도입. 도입 Gate는 [Execute Guide](../07_EXECUTE.md) §17 | [ADR-0024](./0024-execute-v1-execution-model.md) §3 |
| cancelled/timeout attempt 상태 | 존재 (stall scope, cancel) | 동기 fake에 발생 경로 없음 — concrete adapter(Phase 5)와 함께 | [ADR-0024](./0024-execute-v1-execution-model.md) §4 |
| capability의 실제 차단 | runtime 계층에서 tools 목록·approval mode 전달 | v1은 계약 전달·기록까지. 차단은 Phase 5 adapter | [ADR-0024](./0024-execute-v1-execution-model.md) §6 |
