...
>
> **2026-08-09 (Phase 8 종료 검토) — 두 번째 도과.** skill 어디에도 동시 쓰기
> 충돌 절차가 없다. **세 번째 재지정을 하기 전에 이유를 적는다**: 두 번 밀린
> 원인은 시한을 "구현 Phase"에 붙였기 때문이다 — 조건(동시 writer)은 Phase 7이
> 만들었지만 **실제 충돌이 관측된 적이 없고**, 관측 없이 문안을 쓰면 그것은
> 발명이다(upstream 대응물 없음).
>
> **Phase 9로 재지정하되 발동 조건을 함께 건다** — 실사용에서
> `StaleWriteError`가 한 번이라도 관측되면 그때 문안을 쓴다. 관측되지 않으면
> Phase 9 종료 검토에서 **"발생하지 않는 문제"로 닫는 것도 정당한 처분**이다.
> 무한히 미루지 않는다
> ([progress 0008](../progress/0008_PLUGIN_COMPOSITION_LAYER.md) §2.2).
