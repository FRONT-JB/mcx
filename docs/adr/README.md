...
| [0045](./0045-worktree-isolation-contract.md) | worktree 격리 — 미션 하나에 worktree 하나(`mcx/<mission_id>`), 사용자 checkout에 변경이 쌓이지 않는다. 경로는 **저장하지 않고 유도한다**(upstream `TaskWorkspace` 직렬화 없음) — 역사적 기록은 attempt의 envelope가 이미 들고 있다. 새로 만들 때만 clean checkout을 요구하고, git 저장소가 아니면 격리 없이 그대로 간다. 되돌려 합치지 않으므로 **위치 표시가 계약의 일부**다. lock은 pid 생존만 보고 시간 staleness는 버린다. 정리는 **하지 않는다**(등록된 divergence, 발동·종료 조건 부착). | Proposed |
