...
| [0052](./0052-parallel-execution-introduction-gate.md) | 병렬 실행 도입 Gate — unknown을 독립성 증거로 쓰지 않는다. durable stage plan·shared-write conflict authority·grouped attempt·resume·bounded fan-out·실경로 검증 전까지 implementation은 `HOLD`; 현재 순차 실행 유지. | Accepted |
