...
- **첫 라운드 커밋의 굵기** (§3의 Cost). 실사용에서 이것이 rollback을 무용하게
  만드는지는 관측이 있어야 안다. → **닫음** ([Phase 9 종료 검토](../progress/0009_RECOVERY_LAYERS.md) §3-6, 2026-08-10):
  도그푸딩 0005의 첫 checkpoint가 AC 6개의 변경을 통째로 담았음에도 두 번째
  라운드의 rollback은 **실패한 시도만** 지웠다. 굵은 첫 커밋이 rollback을
  무용하게 만들지 않는다는 것이 실물로 확인됐다 — *"Execute 중 임시 커밋 후
  squash"* 후보는 폐기한다.
- **실패한 checkpoint의 재시도** (§5의 divergence). upstream은 기억하고 우리는
  다시 시도한다. → **확정** ([Phase 9 종료 검토](../progress/0009_RECOVERY_LAYERS.md) §3-7, 2026-08-10): 도그푸딩 0005에서 checkpoint가
  **실제로 실패**했고(경로 파서 버그) 고친 뒤 다음 호출이 그대로 커밋했다.
  upstream처럼 기억했다면 그 AC들은 영원히 커밋되지 않았다 — **divergence가
  옳았다는 실물 증거**이며 유지한다.
