...
**Phase 11 actual CLI trace 최종 처분 (2026-08-10).** 한 답변에 429·503·표시
형식·외부 의존성 금지를 함께 넣자 파생 후보 하나가 원문 전체를 보존했다. 후보가
굵은 것은 실제로 관측됐지만 쪼개면 LLM 판단을 새로 넣어야 하고 pinned upstream도
원문 전체를 보존하므로 **현 계약을 유지한다**. 반면 같은 section·같은 원문을
`brief candidate`로 다시 넣자 두 번째 승격 후보가 생겼다. 수동 경로만 가진
우리 divergence가 만든 실패이며, downstream에 같은 성공 조건을 두 번 투영할
근거가 없다. 따라서 `section + 양끝 공백을 제거한 exact text`가 이미 있으면
새 후보를 저장하지 않고 기존 candidate number를 포함한 명시적 오류로 거부한다.
의미 유사도나 문장 분할은 도입하지 않는다.
