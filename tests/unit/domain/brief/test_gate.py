...
def _ready_brief() -> BriefState:
    """clarity를 제외한 모든 조건을 갖춘 상태."""
    return _answered_brief().approve(statement="이대로 진행해 주세요")
...
class TestClearRequiresEverything:
    ...
    def test_decision_references_the_evaluated_revision(self) -> None:
        ...
        decision = evaluate_brief_gate(
            state=state, assessment=_assessment(), policy=POLICY, stability_signal=2
        )
    ...
    def test_decision_records_policy_version(self) -> None:
        ...
        decision = evaluate_brief_gate(
            state=_ready_brief(), assessment=_assessment(), policy=POLICY, stability_signal=2
        )
    ...
    def test_hold_has_no_next_destination(self) -> None:
        decision = evaluate_brief_gate(
            state=_answered_brief(), assessment=_assessment(), policy=POLICY, stability_signal=2
        )
...
class TestApprovalIsNotSufficient:
    ...
    def test_material_unresolved_item_holds_despite_approval(self) -> None:
        ...
        state = _ready_brief().note_unresolved(
    ...
    def test_non_material_unresolved_item_does_not_hold(self) -> None:
        state = _ready_brief().note_unresolved(description="버튼 색상 미정", is_material=False)
    ...
    def test_completion_candidacy_alone_is_not_clear(self) -> None:
        ...
        candidacy = POLICY.assess_completion(
            assessment=_assessment(), answered_rounds=len(state.rounds), stability_signal=2
        )
...
class TestClarityBlockersArePreserved:
    ...
    def test_missing_assessment_holds(self) -> None:
        decision = evaluate_brief_gate(
            state=_ready_brief(), assessment=None, policy=POLICY, stability_signal=2
        )
    ...
    def test_blocking_reasons_are_human_readable(self) -> None:
        decision = evaluate_brief_gate(
            state=_answered_brief(), assessment=None, policy=POLICY, stability_signal=0
        )
